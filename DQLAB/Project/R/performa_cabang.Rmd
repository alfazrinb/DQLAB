...
__geom_type__ diganti dengan fungsi sesuai dengan jenis plot yang diharapkan, misalnya __geom_line, geom_bar, geom_point, geom_boxplot__ dan lainnya.
...
Untuk melihat data bulan Mei 2020, gunakan fungsi filter untuk memfilter data __df_loan__ untuk tanggal dari awal mei **'2020-05-01'** sampai dengan akhir mei **'2020-05-31'**.
...
Tampilkan 5 cabang terbesar dari data __df_loan_mei__,
...
Gunakan fungsi ```comma``` dari package ```scales``` untuk menampilkan __total_amount__ agar lebih mudah dibandingkan.

sebelumnya load package ```scales``` dengan cara,

```library(scales)
  ```
Jangan lupa gunakan pipe __%>%__ untuk menyambungkan fungsi.
...
#### Certificate 
![](certificate-DQLABPRJ8 RVRKVB-1.png)